...
use sp_runtime::{
	...
	traits::{
		...
		SaturatedConversion, StaticLookup, Convert,
	},
...
impl<R> OnUnbalanced<NegativeImbalance> for DealWithSlash<R>
{
	...
	// 	// }
	// }
	fn on_nonzero_unbalanced(mut fees_then_tips: NegativeImbalance) {
		
			info!("slash on_nonzero_unbalanced occur ");
			// debug!("{}",fees_then_tips);
...
where   
	...
	<R as pallet_balances::Config>::Balance: Display
	// <R as frame_system::Config>::AccountId: From<AccountId>,
	// <R as frame_system::Config>::AccountId: Into<AccountId>,
{
	fn on_nonzero_unbalanced(mut reward: PositiveImbalance<R>) {
		let val = reward.peek();
			info!("reward on_nonzero_unbalanced occur {} -- data", val);
			// debug!("{}",val);
	}
} 
...
impl pallet_staking::Config for Runtime {
	...
	type Event = Event;
	type Slash = DealWithSlash<Self>; // send the slashed funds to the treasury.
